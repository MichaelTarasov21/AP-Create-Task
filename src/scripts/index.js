...
function showhistory(guesses, numbertoguess) {
	guesses.forEach(function (item, index) {
		if (item < numbertoguess) {
			document.getElementById("Guesses").insertAdjacentHTML(
				"beforeend",
				`<br>${index + 1}: 
...
function loadsettings() {
  ...
  type="number" min="2"><br><input type="checkbox" id="Limitoption"><label>Limit number of guesses</label><div id=guesslimitbox></div><br>
  <input type="submit" value="Start the game" id="start_button"></div></div>`;
	...
	document.getElementById("start_button").addEventListener("click", function () {
		...
		if (CheckIfIntInput(maxnumber, true) || maxnumber <= 1) {
		...
		if (guesslimit) {
			...
			if (CheckIfIntInput(guessamount, true) || guessamount <= 0) {
...
function startgame(maxnumber, guessamount = 0) {
	...
	if (guessamount !== 0) {
		guesslimit = true;
		document.getElementById("tips").insertAdjacentHTML("afterend", `<br></div><div id="guessesleft">Guess ${guesses.length + 1} of ${guessamount}</div>`);
	}
	document.getElementById("submitguess").addEventListener("click", function () {
		...
		if (CheckIfIntInput(guess)) {
			...
			if (guess == numbertoguess) {
				...
				showhistory(guesses, numbertoguess);
				return;
			...
			if (guesslimit) {
				document.getElementById("guessesleft").innerHTML = `</div><div id="guessesleft">Guess ${guesses.length + 1} of ${guessamount}</div>`;
